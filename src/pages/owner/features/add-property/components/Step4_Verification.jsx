...
const Step4_Verification = ({ formData, errors, handleDocUpload }) => {
  const DocumentItem = ({ id, name, label, subtitle, error }) => {
    ...
    const displayFileName = formData[name]?.fileName || (typeof formData[name] === 'string' ? formData[name] : '');
    ...
    return (
      <div className={`${stepStyles["document-upload-item"]} ${error ? stepStyles["has-error"] : ''}`}>
        <div className={stepStyles["doc-header"]}>
          ...
          <span className={`${stepStyles["doc-badge"]} ${isUploaded ? stepStyles["uploaded"] : stepStyles["pending"]}`}>
            {isUploaded ? <CheckCircle size={14} /> : (
        ...
        >
          ...
          <p className={stepStyles["upload-title"]}>{displayFileName || 'Click to upload or drag and drop'}</p>
